...
# [Executed at: Mon Oct 16 16:27:11 PDT 2017]
#
# Base Link Transform: PASSED
# Link 3 Transform: PASSED
# Link 7 Transform: PASSED
...
class ForwardKinematics(object):
    ...
    def compute_transforms(self, link_names, joints, joint_values):
        ...
        # YOUR CODE GOES HERE
        linknames_len = len(link_names)

        for i in xrange(linknames_len):
            ...
            joint_origin_xyz = joint.origin.xyz
            joint_origin_rpy = joint.origin.rpy
            joint_angle = 0 # joint value is the joint angle

            for j in xrange(len(joint_values.name)):
                if joint_values.name[j] == joint_name:
                    joint_angle = joint_values.position[j]
                    break

            quaternion_1 = tf.transformations.quaternion_from_euler(
                            joint_origin_rpy[0], joint_origin_rpy[1], joint_origin_rpy[2])

            if (joint_type == "revolute"):
                quaternion_2 = tf.transformations.quaternion_about_axis(joint_angle, joint_axis)

                T = tf.transformations.concatenate_matrices(T,
                        tf.transformations.quaternion_matrix(quaternion_1),
                        tf.transformations.translation_matrix(joint_origin_xyz),
                        tf.transformations.quaternion_matrix(quaternion_2))
            ...
            all_transforms.transforms.append(convert_to_message(T, child, parent))
