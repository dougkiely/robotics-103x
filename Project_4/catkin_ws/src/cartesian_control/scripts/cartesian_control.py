...
# [Executed at: Thu Nov 16 16:25:44 PST 2017]
#
# Translation: PASSED
# Rotation: PASSED
# Translation and Rotation: PASSED
...
def cartesian_control(joint_transforms, b_T_ee_current, b_T_ee_desired,
    ...
    b_T_ee_current_inv = numpy.linalg.inv(b_T_ee_current)
    ee_current_T_ee_desired = numpy.dot(b_T_ee_current_inv, b_T_ee_desired)

    b_t_ee = ee_current_T_ee_desired[0:3,3]
    b_R_ee = ee_current_T_ee_desired[0:3,0:3]

    angle, axis = rotation_from_matrix(b_R_ee)
    ...
    V_j_ = numpy.zeros((num_joints, 6, 6))
    ...
    for j in xrange(num_joints):
      ...
      j_T_ee = numpy.dot(numpy.linalg.inv(b_T_j), b_T_ee_current)
      j_t_ee = j_T_ee[0:3,3]
      ee_R_j = j_T_ee[0:3,0:3].T
      V_j_[j][0:3,0:3] = ee_R_j
      V_j_[j][0:3,3:6] = numpy.dot(-ee_R_j, S_matrix(j_t_ee))
      V_j_[j][3:6,3:6] = ee_R_j

      J[:,j] = V_j_[j][:,5]

    threshold_epsilon_val = 0.01
    if red_control:
      threshold_epsilon_val = 0.0

    J_plus = numpy.linalg.pinv(J, threshold_epsilon_val)

    dq = numpy.dot(J_plus, v_ee)

    if red_control:
      q_dot = numpy.zeros(num_joints)
      q_dot[0] = q0_desired - q_current[0]
      dq_n = (numpy.identity(len(q_dot)) - J_plus.dot(J)).dot(q_dot)
      dq = dq + dq_n
